...
* `wait_timeout` (string) - The amount of time to wait for a resource to be created or deleted (defaults to ten minutes, `"10m"`). Valid time units are `"ns"`, `"us"` (or `"µs"`), `"ms"`, `"s"`, `"m"`, and `"h"`.
...
source "magalucloud" "example" {
  api_key      = var.api_key
  region       = "br-se1"
  source_image = "cloud-debian-12 LTS"
